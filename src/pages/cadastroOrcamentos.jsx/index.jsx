...
function CadastroOrcamento() {
 ...
 const [orcamentoValido, setOrcamentoValido] = useState(false)
  ...
  const addDesconto = (index, desconto) => {
    ...
    if (desconto > produto.preco) {
      ...
      let calculo = produto.qtd * produto.preco - produto.desconto;
      produto.totalProduto = calculo.toFixed(2);
      setProorca(novosProdutos);
      calcularTotal();
  ...
  useEffect(() => {
    ...
    if (orcamento == undefined) {
      ...
      axios.post('http://192.168.237.130:3000', orcamento)
...
{produtoInvalido && (
      ...
      {enviar && (
        <Alert variant="success" onClose={() => setOrcamentoValido(false)} dismissible>
          Orcamento enviado! Digite um novo orçamento.
        </Alert>
      )}
          ...
          <Table striped bordered hover>
            ...
            <thead>
              <tr>
                ...
                <th id="tdDescrição"> descontos
                   <Badge bg="primary" pill>
                    R$: {calcularTotalDesconto().totalDosDescontos} </Badge> 
                </th>
